...
Ahora que terminamos de revisar nuestros supuestos *a priori*, utilizaremos los datos observados de adopciones en nuestra base de datos filtrada.
...
El intervalo de credibilidad del 95% para alpha\[1\] (gatos negros) es \[3.73, 3.83\].

El intervalo de credibilidad del 95% para alpha\[2\] (otros colores) es \[3.61, 3.65\].
...
La media posterior de alpha\[1\] (gatos negros) es 3.78.

La media posterior de alpha\[2\] (otros colores) es 3.63.
...
```{r}
lambda_black <- exp(-3.78)  # Tasa de adopción para gatos negros
lambda_other <- exp(-3.63)  # Tasa de adopción para gatos de otros colores
...
ggplot(simulated_data_long) +
  geom_density(aes(x = days_to_event_sim, fill = "Simulado"), alpha = 0.5) +
  geom_density(aes(x = observed, fill = "Observado"), alpha = 0.5) +
  labs(x = "days_to_event", y = "Densidad") +
...
ggplot(filtered_data) +
  geom_density(aes(x = days_to_event_sim, fill = "Simulado"), alpha = 0.5) +
  geom_density(aes(x = observed, fill = "Observado"), alpha = 0.5) +
  labs(x = "days_to_event (100 días)", y = "Densidad") +
  guides(fill = guide_legend(title = "Type")) +
  theme_minimal() +
...
plot_data <- sims_check_post |>
  mutate(type = "Simulado") |>
  bind_rows(AustinCats_model_1 |> 
              mutate(rep = "Observado", days_to_event_sim = days_to_event, type = "Observado")) |>
...
ggplot(plot_data, aes(x = days_to_event_sim, color = type)) +
  ...
  labs(x = "days_to_event", y = "Densidad", color = "Type") +
  scale_color_manual(values = c("Simulado" = "skyblue", "Observado" = "orange")) +
  theme_bw() +
...
sim_gatos_2 <- function(lambda_negro = 0.01, lambda_otro = 0.02, N = 1000, p = 0.15, p_transfer = 0.1) {
  ...
  # Determinar si cada gato es transferido
  final_days_to_event <- ifelse(is_transferred, pmin(days_to_event, transfer_days), days_to_event)
  
  # Simular el estado de censura: 0 = censurado (censurado a la derecha o transferido), 1 = adoptado
...
La CCDF (Complementary Cumulative Distribution Function) muestra la probabilidad acumulada complementaria, es decir, la probabilidad de que un gato no sea adoptado antes o en un tiempo determinado.
### Pruebas del proceso estadístico
...
Nuevamente, utilizaremos una muestra simulada de 1000 gatos de nuestro proceso generador de datos para hacerle pruebas *a piori* a nuestro modelo.
...
Ahora que terminamos de revisar nuestros supuestos *a priori*, utilizaremos los datos observados de adopciones en nuestra base de datos completa.
...
$$log \mu_i = \alpha_{Color_i}+\beta_{raza}$$ $$\alpha_{Color} \sim N(0,1)$$ $$\beta_{raza} \sim N(0,1)$$
...
Vemos que el porcentaje de gatos negros se distribuye similar entre la raza "Domestic Shorthair Mix" y "Other", a como se distribuyen en general.

```{r}
resultados <- AustinCats %>%
  count(raza, pelaje) %>%
  group_by(raza) %>%
...
Vemos que los tiempos de adopción para color siguen siendo desfavorables para los gatos de color negro; sin embargo, si nos fijamos en la variable de raza los tiempos de adopción son muy similares, ligeramente más largos para la raza "Domestic Shorthair Mix", para verificar esto, correremos únicamente para la clasificación binaria "Domestic Shorthair Mix" y "Other" el modelo de datos censurados.
...
library(purrr)
CATS <- AustinCats
CAT_RAZA_ID <- as.integer(as.factor(ifelse(CATS$breed != "Domestic Shorthair Mix", "Other", CATS$breed)))
CAT_RAZA <- levels(as.factor(ifelse(CATS$breed != "Domestic Shorthair Mix", "Other", CATS$breed)))
...
data {
  ...
  real<lower=0> days_to_adoption[N]; // time to event
  int<lower=1, upper=K> cat_raza_id[N]; // categorical raza id
  real<lower=0> lambda;         // rate parameter
  real<lower=0> right_censoring[N];  // right censoring times
}

parameters {
  vector<lower=0>[K] alpha;     // scale parameters
}
...
model {
  ...
  for (n in 1:N) {
    if (days_to_adoption[n] == right_censoring[n]) {
      target += exponential_lccdf(right_censoring[n] | 1 / alpha[cat_raza_id[n]]);
    } else {
      days_to_adoption[n] ~ exponential(1 / alpha[cat_raza_id[n]]);
    }
  }
}
"
...
# Supongamos que ya tienes el objeto fit después de ajustar el modelo
posterior_samples <- rstan::extract(fit, pars = "alpha")$alpha

# Convertir las muestras a un data frame para facilitar la manipulación
posterior_df <- as.data.frame(posterior_samples)

# Calcular los intervalos de credibilidad (High Density Intervals, HDIs)
library(coda)

hdi <- apply(posterior_df, 2, function(x) {
  HPDinterval(as.mcmc(x), prob = 0.89)
})

# Calcular la media de los parámetros
means <- apply(posterior_df, 2, mean)

# Preparar los datos para ggplot2
plot_data <- data.frame(
  parameter = factor(1:ncol(posterior_df), labels = paste0("alpha[", 1:ncol(posterior_df), "]")),
  mean = means,
  hdi_lower = hdi[1, ],
  hdi_upper = hdi[2, ]
)

# Paso 2: Crear el gráfico forest plot

library(ggplot2)

ggplot(plot_data, aes(x = parameter, y = mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = hdi_lower, ymax = hdi_upper), width = 0.2) +
  coord_flip() +  # Girar el gráfico para que sea horizontal
  labs(x = "", y = "days_to_adoption") +
  theme_minimal() +
  theme(
    axis.text.y = element_text(size = 8),
    axis.title.y = element_blank(),
...
```
Y vemos que, en efecto, los días de diferencia entre razas son menores a los tiempos de espera considerando la variable de color.

Edades

Intentamos realizar un ejemplo con edades; sin embargo, al manipular los datos, nos dimos cuenta de que al parecer hay un error de captura en la variable intake_age, creemos que algunos datos pudieron haberse capturado cómo meses y otros en años.

```{r}
mese_anios <- AustinCats %>%
    mutate(anios_calculado = round(intake_age / 12, 0)) %>%
    select(intake_age, anios_calculado,id_pelaje) %>%
    group_by(anios_calculado) %>%
    summarize(n_gatos = n(), num_gatos_negro = sum(id_pelaje == 1, na.rm = TRUE)) %>%
    mutate(pct_muestra = round(cumsum(n_gatos) / sum(n_gatos) * 100, 1)) |>
    mutate(pct_g_negros=round(num_gatos_negro/2965,1)*100)

mese_anios
```

Donde entre 0-1 años se concentra el 80% de los gatos totales, y 90% de los gatos negros.



# Conclusiones

Como pudimos ver para el modelo sin datos censurados, los gatos de pelaje negro tienen tiempos de espera para ser adoptados más largos que el resto.
El modelo con datos censurados, nos mostró el mismo comportamiento, pero nos da una predicción más confiable al no tener el sesgo de estar mirando solo los tiempos para los gatos que sí fueron adoptados y, en cambio, también ver que hubiera pasado con su adopción si otro evento no hubiera sucedido antes.

El tercer modelo, que además de tomar en cuenta los datos censurados añade la variable de raza, nos muestra el mismo comportamiento para el color, sin embargo, nos ayudó a visualizar que, al menos para los gatos, la raza no es un factor tan relevante o discriminatorio en las adopciones.
